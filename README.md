...
Note: Context windows are large, but large datasets are still larger than large context windows.
...
# Steps to generate data (or use pre-made "sample" zip-archive):
##### Whether in google colab, run locally, or another environment:
1. Setup
- make your python-env (unless it already exists as with google Colab)
- put these files together:
- - generator notebook
- - validation checking notebook
- - documentation txt file
- configure parameters if you want or need to in the generator notebook (e.g. size of synthetic dataset)

2. Run Notebook ('synthetic_data_testing_generation_tools')

3. optionally inspect with the validation checking notebook

4. find the html file:
- check the validation health test results
- administer the test to an AI model

5. record results in your results .csv file
...
## Appendix 1: Data Splits

#### Why Train-Test-Validate splits are (possibly) not needed, current thinking:

1. We are testing 'analysis' not 'prediction': The questions ask about identifying existing patterns in data, not forecasting future outcomes.

2. The data is ~deterministically generated (same-input-same-output) with known patterns.

3. Validation is not predictive accuracy: We are confirming that patterns were properly generated, not how well they generalize (predict).

4. The testing scenario simulates data analysis workflows where analysts work with complete datasets, not where models predict future or unseen data.


## Questions involving Prediction
- possible prediction questions:
-- flea treatment effectiveness - nutrition, medicine, etc. factors involving weight height color age etc.
- having a 'train' 'test' sort of split, where a first set would be shown to the AI to predict what happens in a second not-shown set of data, which is the same pattern continuing.
- possibly testing how much/many data are needed for correct or better predictive results
- questions of validating answers
