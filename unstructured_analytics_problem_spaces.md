Evaluating Vector & Hybrid Data Analytics
...
2026.02.26-03.08 G.G.Ashbrook


## Introduction

Especially given the rise of deep learning, foundation models, generative models and "embedding" concept-vector models, it is important to define what you are doing clearly enough to identify what the most appropriate tools are for your project.*

Consistent with the history of STEM (including project-management) whenever we are asking a question, or posing a query, it is important to understand and to define our questions using feedback from tests and evaluations in an iterative workflow, not shooting from the hip once and betting that we hit the right something. 

#### There must be:
1. Definition
2. Feedback (including tests and experiments)
3. Use of data from feedback
4. Iteration (improving the definition in step one, etc.)

This report articulates some of the steps, factors, starting organization, and other suggestions, if only at a cursory starting depth (considering the vastness of this topic), using a classic-generic-context of 'cats and animals on the internet' for the data.

There are also several code-tools in python3 that you can use (likely modified by you for your own project) that go along with this report for you to use to get the information that you will need to define your questions using the feedback-data that will come from the tests and evaluations that you craft.


*(See: github.com/lineality/known_class_classification_workflow_datascience )
...
Preliminary results from these tools and this type of analysis support most expected results, such as that more nuanced and scaled-up/compounded queries produce fuzzier results, and that smaller older models perform noticeably worse at specific tasks (such as recognizing pedantic numbers rather than common class labels (for generative-model data structuring)). 

This aims to be a very constrained-narrow but concrete base-camp from which a very large number of possible and modularly recombinant questions can be extended. 

## MVP-1 Code-Utilities Outline:
https://github.com/stemnetbenchmarks/synthetic_biology_datasets_python

1. Synthetic Data Generation & Processing
2. Vector Database for Testing

3. Single Query Tests: Notebook to compare the results of the four types of 
- 5 tests with granular reporting
- HTML-dashboard of results (and .csv results files)

4. Comparative Query Tests: 
- A-D (not E), comparing stronger signal for Cat vs. Dog
- e.g. "Are there more Cat-etc or Dog-etc results?"

5. Data Extraction/Structuring Evaluation
- A. iterative crawl (one row at at time)
- B. multi-batch (batches of rows)
...
## Specific Evaluation Tests & 'Types' of Datasets

It is important to understand some basic types of testing/training/validating data when you are putting together your project, tests, data, and datasets. (Remember, standard wisdom has long held that this is the first 90% of work in most projects (though that endlessly surprises people).) 

A classification type test against all rows (common in traditional statistics where there is an emphasis on dataset properties such as "balanced" or "imbalanced" datasets) is not the same as a 'unique recommendation' or 'unique data row' type test that is based on "ranked" uniqueness. A rank-based (unique row finding) test will not work to evaluate a large class-label dataset (of non-unique rows) for example 10 million rows of labeled hot-dog or not-hot-dog data. 

'Rank' Type datasets and tests are for where the context is highly unique rows, often unique 'unstructured' data such as book-text or images (e.g. finding the one matching page in a children's picture book, or in a large PDF). Having half the dataset being a correct match will output worse than meaningless results: for traditional statistics on field values the larger the dataset the better, but if you put N-million rows of hot-dog labels into a rank test, the bigger (traditionally 'better') your balanced dataset is the more distorted the results will be. 

Examples: 

1. Class-Label Type: Per Row Confusion Matrix Data (TP,TN,FP,FN (Type-1 and Type-2 errors) & Confusion Matrix Derivative-Metrics (Precision, Recall, F1 (balanced between precision and recall), there are more than 50 derived metrics for various specific uses.)

2. Rank Type: Recall@K, e.g. Single unique item in Top-10 (not a class/classification label) 

3. Rank Type: Rank Analysis: MRR (Mean reciprocal rank), Mean Rank, Median Rank


As another item that is a repeated snag for projects: model performance is dataset specific. You cannot train or test a system on one 'dummy (not real-world) dataset' and in any way reliably expect that system to perform just as well on new and different real-world data-sets. 

Also, note that a 'pre-trained' model (it's in the name) is not able to be updated and trained over time as other forms of learning-models are updated and trained on new and current data (in a previous age 'benchmarking' usually meant evaluating ongoing training & performance of a (not one time pre-trained) model in production-deployment (being used by real world users)). 

Highly unique fuzzy requests for highly unique fuzzy unstructured data are peripherally mentioned here, but the main focus is on leveraging what is well defined. Even where a given semi-unique combination of structured fields is in scope, it is not clear that this justifies using a 'must be unique or the results are meaningless' rank-test.
...
While raw-vector models and generative models may be identical except for the 'head' of the output, it is likely that in the early 2020s we did not understand more deeply how the two work and differ. 
...
- Do hardware condition fluctuations affect vector models the way they affect generative models?


## 25. On the fly vector analytics
On the fly vector analytics vs. a classic stored vector database:
One of the drawbacks of using vector analytics for some projects is the technical, time, and financial overhead of maintaining a scary sounding 'vector database.' However, some of this concern is people getting tripped-up over their own flourishes and attempts to razzle-dazzle customers with amazing-sounding descriptions. While you can invest a lot of time and money in a 'super shiny' third-party-maintained deluxe "we can take care of that for you for a price but our API is tricky" low-code no-code service:
rolling your own 'vector database' is trivial and can be done 'on the fly' in at least two ways.
1. Not all vector queries require a permanent, maintained, always-on "database" that exists as cloud infrastructure. You can perform a vectorization of the records that you need in seconds or minutes, if the query does not need to be lighting fast.
2. A 'database' of stored vectors does not need to be a fancy expensive infrastructure project. Again, if the system does not need to be a lightning fast cloud service 24-7, 365, globally online, you probably have better options custom made by you for your needs (ideally using something like Rust and not Python). 
## 26. Could vector-search and selective-document-structuring be combined? 
A key reason to not do document structuring is the time/resource cost for what might be a (multiple attempts at a) single query (not a recurring query). But could the number of documents being structured first be narrowed down by some combination of vector and tabular filters? This might further extend the precision obtained when using extraction of tabular data from an unstructured document.

## 27. How long does it take to do structuring/extraction for a query, with a generic foundation model, with an optimal-small model, with a custom-made model? Can that time/cost be made small enough to be practial? (note: pre-filter to reduce the number of documents structured/extracted may further optimize this). 
...
How can clustering-dispursion of results potentially be used?


# 2. Phylogenies / Taxonomies of Analytics:

The following are preliminary attempts at breaking down areas for use, testing, and comparison (e.g. comparing what approach works better at a given task (or can do/support a task at all), and or what limitations of an approach are).

#### Vector-Query-Data Analysis:
One of the primary ideas to test, is using the data from the results of a vector-database query as the hard-data with which to perform data-analytics on unstructured data (such as unstructured, test, images, audio, or multi-media/multi-modal 'documents' in a 'corpus').
...
## Mechanics Types:
A. Single Query
Ways use data-results of one vector query/search as primary data to analyze to gain information about the dataset:
- How many chunks were matched to
- Cross-document-strength (e.g. how many chunks are within N distance from target)
...
- How clustered
- How dispersed
- Distance-Score of the top result specifically

B. Comparing N Queries (e.g. 2)
...
## 'counts vs. presence/existence vs. relative prominence': feasibility
...
Probably tricky, but possibly an area for progress.
...
11. Future State and Configuration Predictions
A. What would happen if __?
B. Red, Blur, Purple Team: What might someone do...and then what might someone else do?

# Datasets & Functionalities

As per the focus of this study/report, defining exactly what question/query/functionality is in scope for your project is crucial.

While it is possible that a single dataset, using various fields in the same rows, may be both usable and desirable for (testing, and potentially 'training' for) multiple "features," it should be remembered that, potentially, each unique feature may require both a separate dataset and testing apparatus fit for that specific "feature." "Feature" or "functionality" are common, more general, terms for what exactly the project is doing. In this case the feature or functionality will often be a query, but processes such as structuring/extracting data fields should also be (features/functionalities that are) tested/evaluated. (Sadly the term 'feature,' which is often used to refer to user-level user-features (thing that the user can do) can confusingly collide with the same term 'feature' that refers to fields or columns in a dataset (such as in 'feature engineering').

There may also be other factors such as, what specific edge cases do you want to test for? 

Without having robust answers to these questions, you cannot reliably slop together a general data pile that will be an asset. There is a reason why the rule of thumb that for any DS AI ML project continues to be true decade after decade: the first 90% of time and energy goes into dataset management before you can even start anything else.


# MVPs & MVP-1

Before running the tests, existing python code is used to generate data needed (at the user specified quantity) and the specified unstructured blurbs. 

Tests are carried out using the .csv data that includes the unstructured_description field. Some tests only use the tabular data, some tests only use the vectorized unstructured blurb, some tests use both (hybrid).

After running tests python code is used to analyze the results and to produce an HTML dashboard of results.
...
- This is about a flying cat: a feline cat who can fly, the cat weighs over N kg. The cat is shorter than N cm, and has more than N friends. This can can [fly,swim,run], watches youtube and eats more than N grams of food per day.
A. to E.
...
6. How reliable can document structuring be, for what cost?
...
If a match fails, we should also check to see which row might match most closely. If there is a best-fit row,
...
- Vector count vs. tabular count
- Confusion Matrix Analysis: Precision, Recall, F1, Specificity, proportions of field errors, etc.
- Time-cost analysis
- Absolute error
- Summary Tables (.csv & HTML Dashboard)

### Comparison Analysis
- Count comparison
- ratio-comparison (cat/dog)
- Confusion Matrix Analysis
- Summary Tables (.csv & HTML Dashboard)


### Visualization of Vector Space
- 3D Comparison: Cat vs. Dog plots
- 3D A-E confusion matrix plots
- A-E Comparison Plot


## Extraction/Structuring from unstructured to structured form
#### Row Test
- Row level matching stats
...
# Datasets (including future-goals for datasets):
...
This project uses Mistral and Anthropic models, but for api-model testing and for generating and checking mvp prototype code.
This report was not, nor any section of it, generated by model.
